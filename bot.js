...
bot.hears('roboruri ping', (context) => {
...
bot.command('/mute', (context) => {
	//Roboruri will leave if: the user who requested is a group admin or the bot developer
	context.getChat().then((chat) => {
		if(chat.type === 'supergroup' || chat.type === 'group') {
			context.getChatAdministrators().then((admins) => {
				if(context.from.id === DEV_TELEGRAM_ID) {
					context.reply('Sayōnara, otōsan');
					MUTED_GROUPS[chat.id] = true;
				}
				for(let i in admins) {
					if(context.from.id === admins[i]['user']['id'] || context.from.id === DEV_TELEGRAM_ID) {
						context.reply('Sayōnara');
						MUTED_GROUPS[chat.id] = true;
					}
				}
			}).catch((err) => {
			...
			context.reply('This command is meant for group chats.');
		}
	}).catch((err) => {
		logger.error(err);
	});
...
bot.command('/unmute', (context) => {

})

// bot.on('text', (msg) => {
// 	msg.getChat().then((what) => {
// 		logger.log(chat.type === 'private');
// 	})
...
// 				logger.warn('[DEBUG]\nmsg: ', msg, '\nGL:', GL);
// 			}
// 			if(msg.text.startsWith('roboruri leavesss')) {
